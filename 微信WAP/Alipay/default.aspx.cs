...
namespace Adpost.YCH.WAP.Alipay
{
    public partial class _default : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            var OrderModel = BOrder.GetViewModel(tmpOrderId);
            if (!string.IsNullOrEmpty(tmpOrderId))
            {
                if (OrderModel != null)
                {
                    if (OrderModel.realamount <= 0)
                    {
                        Response.Write("支付金额必须大于0才能支付！");
                        Response.End();

                    }
                    else if (Convert.ToInt32(OrderModel.orderstatus) == (int)订单状态.已支付)
                    {
                        Response.Write("订单状态不符合请稍候支付");
                        Response.End();
                    }/*
                    else if (Convert.ToInt32(OrderModel.orderstatus) == (int)订单状态.待支付)
                    {
                        Response.Write("订单未确认");
                        Response.End();
                    }*/
                }
            }
            ...
            //服务器异步通知页面路径
            string notify_url = Eyousoft_yhq.AlipayLibrary.Config.GetConfigString("Alipay", "app_notify_url");// "http://www.xxx.com/Alipay/notify_url.aspx";
            //需http://格式的完整路径，不允许加?id=123这类自定义参数

            //页面跳转同步通知页面路径
            string call_back_url = Eyousoft_yhq.AlipayLibrary.Config.GetConfigString("Alipay", "app_callback_url");// "http://127.0.0.1:64704/Alipay/call_back_url.aspx";
            //需http://格式的完整路径，不允许加?id=123这类自定义参数

            //操作中断返回地址
            string merchant_url = Eyousoft_yhq.AlipayLibrary.Config.GetConfigString("Alipay", "app_return_url");
            //用户付款中途退出返回商户的地址。需http://格式的完整路径，不允许加?id=123这类自定义参数

            #region 订单信息|卖家帐号
            //卖家支付宝帐户
            string seller_email = Eyousoft_yhq.AlipayLibrary.Config.GetConfigString("appSettings", "AlipayAccount");
            //必填

            //商户订单号
            string out_trade_no = OrderModel.id;
            //商户网站订单系统中唯一订单号，必填
            ...
            //付款金额
            string total_fee = OrderModel.realamount.ToString();
            //必填
            #endregion
