...
{
    public partial class travel_shopping_show : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (!String.IsNullOrWhiteSpace(Id))
                {
                    ...
                    if (model != null)
                    {
                        this.Talk.CommentType = 评论类型.购物;
                        this.Talk.HotspotId = Id;
                        this.TalkList.CommentType = 评论类型.购物;
                        this.TalkList.HotspotId = Id;
                        this.ltrTalkCount.Text = BComment.Count(Id, 评论类型.购物);
                        InitRoomInfo(Id);
