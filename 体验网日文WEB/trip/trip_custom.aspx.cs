    ...
    public partial class trip_custom : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (UserModel == null)
                {
                    MessageBox.ShowAndRedirect("用户未登陆，请登陆后再进行行程定制!", "/Login.aspx");
                    return;
                }
                else
                {
        ...
        private void InitHotelList()
        {
            ...
            if (list != null)
            {
                ...
                for (int i = 0; i < list.Count(); i++)
                {
                    if ((i + 1) % 2 == 0)
                    {
                        tmpStr.Append("<div class=\"list_div right\"><img class=\"list_img\" src=\"" + Common.NoPhotoDefault(list[i].coverphoto) + "\"><h1>" + list[i].hotspot_name + "</h1><a href=\"javascript:void(0)\" ID=\"" + list[i].id + "\" EID=\"" + list[i].hotspot_id + "\" class=\"hotelAdd\">&nbsp;</a></div>");
                    }
                    else
                    {
                        tmpStr.Append("<div class=\"list_div left\"><img class=\"list_img\" src=\"" + Common.NoPhotoDefault(list[i].coverphoto) + "\"><h1>" + list[i].hotspot_name + "</h1><a href=\"javascript:void(0)\" ID=\"" + list[i].id + "\" EID=\"" + list[i].hotspot_id + "\" class=\"hotelAdd\">&nbsp;</a></div>");
                    }
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (days.Length > 0)
                {
                    ...
                    MessageBox.ShowAndRedirect("行程订制成功!", "trip_custom.aspx");
                    return;
                }
                else
                {
